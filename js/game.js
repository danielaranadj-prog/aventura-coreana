...
function spawnFirework() {
  ...
  fireworks.push({ particles, exploded: true });
}
...
function updateWinPreview() {
  ...
  if (winPreviewTimer === 0 && Math.random() < 0.15) {

    spawnFirework();

  }

  // Dibujar fuegos artificiales sobre el canvas del juego
...
// Muerte por caer en hueco (Reemplaza tu bloque actual por este)
  ...
  if (player.y > LEVEL_HEIGHT * TILE + 20 || isOverGap) {
    if (player.invincible <= 0) {
      ...
      // Reposicionar de manera segura si ya está parpadeando/invencible
      player.x = 64;
