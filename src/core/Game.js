// ============================================================
// GAME — ORQUESTADOR PRINCIPAL CON DELTA TIME
// ============================================================
...
export class Game {
  ...
  _beginGame() {
    ...
    this.camera = new Camera(this.canvas.width, LEVEL_WIDTH, TILE);

    const groundY = (LEVEL_HEIGHT - 2) * TILE;
    this.player = new Player(64, groundY - 48);
    this.enemies = createEnemies(this.level.map, LEVEL_WIDTH, LEVEL_HEIGHT);
  ...
  _drawStars(ctx) {
    ...
    stars.forEach(s => {
      const parallaxX = (s.x - this.camera.x * 0.1) % (LEVEL_WIDTH * TILE);
  ...
  _drawNebula(ctx) {
    ...
    nebulas.forEach(n => {
      const parallaxX = (n.x - this.camera.x * 0.05) % (LEVEL_WIDTH * TILE);
  ...
  _drawLevel(ctx) {
    ...
    const endCol = Math.min(this.camera.getEndCol(), LEVEL_WIDTH);
  ...
  _drawFinalStructure(ctx) {
    const centerX = (LEVEL_WIDTH - 5) * TILE + TILE;
    const baseY = (LEVEL_HEIGHT - 2) * TILE;
    const w = 112;
    const x = centerX - w / 2;
    ...
    // Nivel 1
    ctx.fillStyle = '#d4c4a0'; ctx.fillRect(x + 8, baseY - 60, 96, 28);
    ctx.fillStyle = '#b8a078'; ctx.fillRect(x + 90, baseY - 60, 14, 28);
    ctx.fillStyle = '#e6d5b8'; ctx.fillRect(x + 8, baseY - 62, 96, 2);
    ctx.fillStyle = 'rgba(0,0,0,0.05)';
    ctx.fillRect(x + 18, baseY - 52, 16, 2); ctx.fillRect(x + 62, baseY - 46, 20, 2);

    // Nivel 2
    ctx.fillStyle = '#c4b088'; ctx.fillRect(x + 16, baseY - 84, 80, 24);
    ctx.fillStyle = '#a89070'; ctx.fillRect(x + 82, baseY - 84, 14, 24);
    ctx.fillStyle = '#d4c4a0'; ctx.fillRect(x + 16, baseY - 86, 80, 2);

    // Nivel 3
    ctx.fillStyle = '#d4c4a0'; ctx.fillRect(x + 24, baseY - 106, 64, 22);
    ctx.fillStyle = '#b8a078'; ctx.fillRect(x + 74, baseY - 106, 14, 22);
    ctx.fillStyle = '#e6d5b8'; ctx.fillRect(x + 24, baseY - 108, 64, 2);

    // Nivel 4
    ctx.fillStyle = '#c4b088'; ctx.fillRect(x + 32, baseY - 134, 48, 28);
    ctx.fillStyle = '#a89070'; ctx.fillRect(x + 66, baseY - 134, 14, 28);
    ctx.fillStyle = '#d4c4a0'; ctx.fillRect(x + 32, baseY - 136, 48, 2);

    // Cúspide
    ctx.fillStyle = '#e6d5b8'; ctx.fillRect(x + 44, baseY - 152, 24, 18);
    ctx.fillStyle = '#c4b088'; ctx.fillRect(x + 62, baseY - 152, 6, 18);
    ctx.fillStyle = '#d4c4a0'; ctx.fillRect(x + 44, baseY - 154, 24, 2);
    ...
    drawWindow(x + 14, baseY - 30, true); drawWindow(x + 38, baseY - 30, false);
    drawWindow(x + 62, baseY - 30, true); drawWindow(x + 86, baseY - 30, false);
    drawWindow(x + 22, baseY - 56, false); drawWindow(x + 50, baseY - 56, true); drawWindow(x + 78, baseY - 56, false);
    drawWindow(x + 30, baseY - 80, true); drawWindow(x + 58, baseY - 80, false);
    drawWindow(x + 38, baseY - 102, true); drawWindow(x + 62, baseY - 102, false);
    ...
    ctx.beginPath(); ctx.moveTo(x + w + 6, baseY - 10);
    ctx.quadraticCurveTo(x + w + 20, baseY - 50, x + w + 10, baseY - 90); ctx.stroke();
